...
namespace HireAI.Service.Implementions
{
    internal class HRDashBoardService : IHrDashboardService
    {
        ...
        {
            var query =   await _applications.GetAll();
        ...
        {
            var applicationQuery = await _applications.GetAll(); //no sql yet
            return await applicationQuery.Where(a => a.ExamSummary != null && a.ExamSummary.TotalScroe <80 && a.ScoreATS<80).CountAsync();
        }

        private async Task<Dictionary<int, int>> GetMonthlyNumberOfApplicationsAsync(int hrId)
        {
            var query = await _applications.GetAll(); //no sql yet
            return await query
        ...
        {
            var query = await _jobOpening.GetAll();
            return await 
